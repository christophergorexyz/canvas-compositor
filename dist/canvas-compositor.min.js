...
(() => {
  ...
  // lib/linear-algebra/vector.ts
  function isVector(arg) {
    return arg instanceof Vector;
  }
  var _Vector = class _Vector extends Array {
    constructor(data = 0) {
    ...
    add(...args) {
      return _Vector.add(this, ...args);
    }
    static add(...args) {
      const length = args[0].length;
      return new _Vector(args.reduce((acc, val) => acc.map((v, i) => v + val[i]), new Array(length).fill(0)));
    }
    multiply(...args) {
      return _Vector.multiply(this, ...args);
    }
    static multiply(...args) {
      const length = args[0].length;
      return new _Vector(args.reduce((acc, val) => acc.map((v, i) => v * val[i]), new Array(length).fill(1)));
    }
    scale(s) {
      return _Vector.scale(this, s);
    }
    static scale(v, s) {
      if (!isVector(v))
        v = new _Vector(v);
      return new _Vector(v.map((val) => val * s));
    }
    /**
     * @todo verify that the parameters read the way order of operations would suggest
     */
    dot(v) {
      return _Vector.dot(this, v);
    }
    static dot(v1, v2) {
    ...
    static magnitude(v) {
      return Math.sqrt(_Vector.dot(v, v));
    }
    normal() {
      return _Vector.normal(this);
    }
    static normal(v) {
      const mag = _Vector.magnitude(v);
      if (mag === 0)
        throw new Error("Cannot normalize zero-length vector");
      return new _Vector(v.map((val) => val / mag));
    }
    subtract(...args) {
      return _Vector.subtract(this, ...args);
    }
    static subtract(...args) {
      const [first, ...rest] = args;
      return new _Vector(rest.reduce((acc, val) => acc.map((entry, index) => entry - val[index]), Array.from(first)));
    }
    cross(v) {
      return _Vector.cross(this, v);
    }
    static cross(v1, v2) {
      return new _Vector([
        v1[1] * v2[2] - v1[2] * v2[1],
    ...
    distance(v) {
      return _Vector.distance(this, v);
    }
    static distance(v1, v2) {
      const diff = Array.from(v1).map((val, i) => val - v2[i]);
      return Math.sqrt(_Vector.dot(diff, diff));
    }
    angleBetween(v) {
      return _Vector.angleBetween(this, v);
    }
    static angleBetween(v1, v2) {
      const dotProduct = _Vector.dot(v1, v2);
      const magnitudes = _Vector.magnitude(v1) * _Vector.magnitude(v2);
      if (magnitudes === 0)
        throw new Error("Cannot find angle with zero-length vector");
      return Math.acos(dotProduct / magnitudes);
    }
    radiansBetween(v) {
      return _Vector.radiansBetween(this, v);
    }
    static radiansBetween(v1, v2) {
      return _Vector.angleBetween(v1, v2);
    }
    degreesBetween(v) {
      return _Vector.degreesBetween(this, v);
    }
    static degreesBetween(v1, v2) {
      return _Vector.radiansBetween(v1, v2) * 180 / Math.PI;
    }
    project(v) {
      return _Vector.project(this, v);
    }
    static project(v1, v2) {
      const scale = _Vector.dot(v1, v2) / _Vector.dot(v2, v2);
      return _Vector.scale(v2, scale);
    }
    static extrema(...args) {
      const length = args[0].length;
      ...
      }, [new Array(length).fill(Infinity), new Array(length).fill(-Infinity)]).map((val) => new _Vector(val));
    }
  };
  __decorateClass([
    sameLength()
  ...
  __decorateClass([
    sameLength()
  ], _Vector, "angleBetween", 1);
  __decorateClass([
    sameLength()
  ], _Vector, "radiansBetween", 1);
  __decorateClass([
    sameLength()
  ], _Vector, "degreesBetween", 1);
  __decorateClass([
    sameLength()
  ], _Vector, "project", 1);
  __decorateClass([
  ...
  // lib/rendering/canvas-2d-render-target.ts
  var Canvas2DRenderTarget = class {
  ...
  // lib/rendering/renderer.ts
  var Renderer = class {
  };

  // lib/rendering/canvas-2d-renderer.ts
  function normalizedReflection(value) {
    return value && value < 0 ? -1 : 1;
  }
  function asBitmapRenderOutput(output) {
    ...
    throw new Error(`Canvas2DRenderer does not support render output kind "${String(output.kind)}".`);
  }
  var Canvas2DRenderer = class extends Renderer {
    createRenderTarget(width, height) {
      return new Canvas2DRenderTarget(width, height);
    }
    presentRenderOutput(output, context) {
    ...
    drawRenderOutput(output, destination, options) {
      const bitmapOutput = asBitmapRenderOutput(output);
    ...
    drawRenderTarget(source, destination, options) {
  ...
  // lib/context-2d/component.ts
  var defaultRenderer = new Canvas2DRenderer();
  var Component = class {
    constructor(width, height, options) {
      ...
      this._effectiveRasterPadding = 0;
      /**
       ...
       * shear
       */
      this._shear = new Vector([0, 0]);
      /**
       * pi π
       * perspective
       */
      this._perspective = new Vector([1, 1]);
      /**
       * reflect across axis
       * these values should always be 1 or -1
       */
      this._reflect = new Vector([1, 1]);
      /**
    ...
    get renderedOffset() {
      ...
      const parentContentOffset = this.parent.contentOffset;
     ...
     */
    reparentTo(targetParent, options) {
  ...
  // lib/context-2d/transform-utils.ts
  var transform_utils_exports = {};
  __export(transform_utils_exports, {
    forwardTransformPoint: () => forwardTransformPoint,
    hitTestComponent: () => hitTestComponent,
    hitTestComposition: () => hitTestComposition,
    inverseTransformPoint: () => inverseTransformPoint,
    localPointForChild: () => localPointForChild
  });
  function childRenderOffset(child) {
    const maybeComposition = child;
    return maybeComposition.contentOffset ?? new Vector([0, 0]);
  }
  function inverseTransformPoint(component, x, y) {
  ...
  function forwardTransformPoint(component, x, y) {
    const pivot = component.rotationPivot;
    const tx = x - pivot[0];
    const ty = y - pivot[1];
  ...
  function localPointForChild(parent, child, x, y) {
  ...
  function hitTestComponent(component, localX, localY, includeStroke = true) {
    return component.isPointInPath(component.path, localX, localY) || includeStroke && component.isPointInStroke(component.path, localX, localY);
  }
  function hitTestComposition(root, x, y) {
  ...
  // lib/context-2d/composition.ts
  var Composition = class _Composition extends Component {
    constructor(width, height, options) {
    ...
    _childRenderOffset(child) {
      if (child instanceof _Composition) {
    ...
    _updateBoundsForChildren() {
      ...
      let maxY = -Infinity;
      for (const child of this.children) {
        const childRenderOffset2 = this._childRenderOffset(child);
        const rasterPadding = child.effectiveRasterPadding;
        const originX = child.displacement[0] + childRenderOffset2[0] - rasterPadding;
        const originY = child.displacement[1] + childRenderOffset2[1] - rasterPadding;
     ...
     */
    reparentChild(child, targetParent, options) {
  ...
  // lib/rendering/compositor-backend.ts
  var CompositorBackend = class {
    constructor(componentRenderer) {
      this.componentRenderer = componentRenderer;
    }
    getPresentationOutput(scene) {
      return scene.getRenderOutput();
    }
  };

  // lib/rendering/canvas-2d-compositor-backend.ts
  var Canvas2DCompositorBackend = class extends CompositorBackend {
    constructor(canvas, componentRenderer = new Canvas2DRenderer()) {
    ...
    _applyComponentTransform(component, context) {
    ...
    _drawComponent(component, context) {
      context.save();
    ...
    _drawCompositionChildren(composition, context) {
  ...
  // lib/compositor.ts
  var Compositor = class extends EventTarget {
    /**
    ...
    _animationLoop() {
      ...
      this.framerate = Math.round(1e3 / (this._currentTime - this._lastFrameTimestamp));
      this._lastFrameTimestamp = +/* @__PURE__ */ new Date();
    }
    /**
  ...
  // lib/context-2d/index.ts
  var context_2d_exports = {};
  __export(context_2d_exports, {
    Bezier: () => Bezier,
    Circle: () => Circle,
    Component: () => Component,
    ...
    Rectangle: () => Rectangle,
    Text: () => Text,
    TextDefaults: () => text_defaults_exports,
    TextUtilities: () => text_utilities_exports,
    TransformUtils: () => transform_utils_exports
  });

  // lib/context-2d/circle.ts
  ...
  var Bezier = class extends Component {
    constructor(options) {
  ...
  // lib/context-2d/ellipse.ts
  var Ellipse = class extends Component {
  ...
  // lib/context-2d/line.ts
  function asVector2(point) {
    return point instanceof Vector ? point : new Vector([point[0] ?? 0, point[1] ?? 0]);
  }
  var Line = class _Line {
    constructor(anchor, direction) {
      this.p1 = asVector2(anchor);
      this.direction = asVector2(direction);
      this.p2 = Vector.add(this.p1, this.direction);
    }
    intersectionWith(line) {
      return _Line.intersection(this, line);
    }
  ...
  // lib/context-2d/path.ts
  var Path = class extends Component {
    constructor(points, options) {
      ...
      const { closed, ...componentOptions } = options ?? {};
      super(width, height, {
        ...componentOptions,
  ...
  // lib/context-2d/picture.ts
  var Picture = class extends Component {
  ...
  // lib/context-2d/polygon.ts
  var Polygon = class extends Component {
    constructor(points, options) {
      if (!points.length) {
  ...
  // lib/context-2d/rectangle.ts
  var Rectangle = class extends Component {
  ...
  // lib/context-2d/text-defaults.ts
  var text_defaults_exports = {};
  __export(text_defaults_exports, {
  ...
  // lib/context-2d/text-utilities.ts
  var text_utilities_exports = {};
  __export(text_utilities_exports, {
    formatFontString: () => formatFontString,
  ...
  var Text = class extends Component {
    constructor(options) {
      const fontString = formatFontString(
        ...
        options.fontVariant ?? FONT_VARIANT,
        options.fontWeight ?? FONT_WEIGHT,
        options.fontSize ?? FONT_SIZE,
        options.lineHeight ?? LINE_HEIGHT,
      ...
      this.fontFamily = options.fontFamily ?? FONT_FAMILY;
      this.fontStyle = options.fontStyle ?? FONT_STYLE;
      this.fontVariant = options.fontVariant ?? FONT_VARIANT;
      this.fontWeight = options.fontWeight ?? FONT_WEIGHT;
      this.lineHeight = options.lineHeight ?? LINE_HEIGHT;
      this.textAlign = options.textAlign ?? TEXT_ALIGN;
      this.textBaseline = options.textBaseline ?? TEXT_BASELINE;
  ...
  // lib/svg/badge.ts
  var SVGBadge = class extends Picture {
    constructor(options) {
  ...
  function getWebGLContext(surface) {
  ...
  // lib/webgl/triangle.ts
  var WebGLTriangle = class extends Picture {
    constructor(options) {
  ...
  // lib/interaction/debug-overlay.ts
  var DebugOverlay = class {
    ...
    render(referenceCanvas, options) {
    ...
    getBoundsQuad(component, boxX, boxY, mode) {
  ...
  // lib/interaction/interaction-controller.ts
  var InteractionController = class extends EventTarget {
    constructor(compositor, options) {
      ...
      this.dragStartDisplacementX = 0;
      this.dragStartDisplacementY = 0;
    ...
    setSelectedComponent(component) {
      ...
      this.dispatchEvent(new CustomEvent("selectionchange", { detail: component }));
    }
    selectAt(x, y) {
    ...
    rotateSelected(deltaRadians, origin = "center") {
      if (!this._selectedComponent) {
        return;
      }
    ...
    scaleSelected(factor, minScale = 0.05) {
      if (!this._selectedComponent) {
        return;
      }
    ...
    setDisplacement(component, x, y) {
      component.displacement = this.withVectorType(component.displacement, [x, y]);
    }
    withVectorType(source, values) {
      const VectorCtor = source.constructor;
  ...
  __export(rendering_exports, {
    Canvas2DCompositorBackend: () => Canvas2DCompositorBackend,
    Canvas2DRenderTarget: () => Canvas2DRenderTarget,
    Canvas2DRenderer: () => Canvas2DRenderer,
    CompositorBackend: () => CompositorBackend,
    RendererBackend: () => Renderer,
    SVGCompositorBackend: () => SVGCompositorBackend,
    SVGRenderer: () => SVGRenderer,
    WebGLCompositorBackend: () => WebGLCompositorBackend,
    WebGLRenderer: () => WebGLRenderer
  });

  // lib/rendering/svg-compositor-backend.ts
  function asBitmapRenderOutput2(output) {
    if (output.kind === "bitmap") {
      return output;
    }
    if (output.kind === "webgl") {
    ...
    throw new Error(`SVGCompositorBackend does not support render output kind "${String(output.kind)}".`);
  }
  var SVGCompositorBackend = class extends CompositorBackend {
    constructor(svg, componentRenderer = new Canvas2DRenderer()) {
      super(componentRenderer);
      this.svg = svg;
    ...
    rasterDataUrl(output) {
      const bitmapOutput = asBitmapRenderOutput2(output);
    ...
    presentRasterizedOutput(output) {
      const bitmapOutput = asBitmapRenderOutput2(output);
  ...
  // lib/rendering/webgl-compositor-backend.ts
  function isTexImageSource(source) {
    return source instanceof HTMLImageElement || source instanceof HTMLCanvasElement || source instanceof HTMLVideoElement || source instanceof ImageBitmap || source instanceof OffscreenCanvas || source instanceof VideoFrame;
  }
  ...
  function createProgram2(gl, vertexSource, fragmentSource) {
    const vertexShader = createShader2(gl, gl.VERTEX_SHADER, vertexSource);
    const fragmentShader = createShader2(gl, gl.FRAGMENT_SHADER, fragmentSource);
    const program = gl.createProgram();
    if (!program) {
      gl.deleteShader(vertexShader);
      gl.deleteShader(fragmentShader);
  ...
  function getWebGLContext2(canvas) {
  ...
  function isIdentityPresentationComponent(component) {
    return component.displacement[0] === 0 && component.displacement[1] === 0 && component.rotation === 0 && component.scale[0] === 1 && component.scale[1] === 1 && component.reflect[0] === 1 && component.reflect[1] === 1;
  }
  var WebGLCompositorBackend = class extends CompositorBackend {
    constructor(canvas, componentRenderer = new Canvas2DRenderer()) {
      super(componentRenderer);
      this.canvas = canvas;
      this.gl = getWebGLContext2(canvas);
      this.program = createProgram2(this.gl, `
      attribute vec2 aPosition;
    ...
    present(output) {
      const source = output.kind === "webgl" ? output.source : output.kind === "svg" ? output.fallbackBitmap.source : output.source;
  ...
  // lib/scene-serialization.ts
  var scene_serialization_exports = {};
  __export(scene_serialization_exports, {
    deserializeScene: () => deserializeScene,
    restoreScene: () => restoreScene,
    serializeScene: () => serializeScene,
    serializeSceneToString: () => serializeSceneToString
  });
  function toVec2(value) {
    return [value[0], value[1]];
  }
  function makeVector(source, value) {
    const VectorCtor = source.constructor;
  ...
  function getTransform(component) {
    return {
      ...
      rotationOrigin: Array.isArray(component.rotationOrigin) ? [component.rotationOrigin[0], component.rotationOrigin[1]] : component.rotationOrigin,
  ...
  function applyTransform(component, transform) {
  ...
  function applyStyles(component, styles) {
    ...
    if (styles.fillStyle !== void 0)
      component.context.fillStyle = styles.fillStyle;
    if (styles.strokeStyle !== void 0)
      component.context.strokeStyle = styles.strokeStyle;
    if (styles.lineWidth !== void 0)
      component.context.lineWidth = styles.lineWidth;
    if (styles.lineCap !== void 0)
      component.context.lineCap = styles.lineCap;
    if (styles.lineJoin !== void 0)
      component.context.lineJoin = styles.lineJoin;
    if (styles.miterLimit !== void 0)
      component.context.miterLimit = styles.miterLimit;
    if (styles.lineDash !== void 0)
      component.context.setLineDash(styles.lineDash);
    if (styles.lineDashOffset !== void 0)
      component.context.lineDashOffset = styles.lineDashOffset;
    if (styles.globalAlpha !== void 0)
      component.context.globalAlpha = styles.globalAlpha;
    if (styles.globalCompositeOperation !== void 0)
      component.context.globalCompositeOperation = styles.globalCompositeOperation;
    if (styles.font !== void 0)
      component.context.font = styles.font;
    if (styles.textAlign !== void 0)
      component.context.textAlign = styles.textAlign;
    if (styles.textBaseline !== void 0)
      component.context.textBaseline = styles.textBaseline;
    component.invalidate();
  }
  function serializeNode(component) {
  ...
  function componentOptionsFor(node) {
    return {
  ...
  async function deserializeNode(node) {
    ...
    switch (node.type) {
      ...
      case "Picture": {
        ...
        const picture = new Picture(image, options);
      ...
      default:
        throw new Error(`Unsupported serialized node type: ${node.type}`);
    }
  }
  function serializeScene(scene) {
    return {
      version: 1,
      scene: serializeNode(scene)
    };
  }
  function serializeSceneToString(scene) {
    return JSON.stringify(serializeScene(scene));
  }
  async function deserializeScene(source) {
  ...
  async function restoreScene(targetScene, source) {
  ...
  globalThis.CanvasCompositor = {
    Compositor,
    Components2d: context_2d_exports,
    ComponentsSVG: svg_exports,
    ComponentsWebGL: webgl_exports,
    Rendering: rendering_exports,
    InteractionController,
    DebugOverlay,
    SceneSerialization: scene_serialization_exports
  };
})();
//# sourceMappingURL=canvas-compositor.min.js.map
